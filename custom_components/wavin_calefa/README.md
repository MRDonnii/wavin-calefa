...
| `source_inlet_temperature` | District heating supply (DHS) | Fjernvarme fremlob (FJF) | C | |
| `source_return_temperature` | District heating return (DHR) | Fjernvarme retur (FJR) | C | |
| `source_delta_temperature` | District heating cooling | Fjernvarme afkoling | C | Calculated: inlet - return |
| `cvv_supply_temperature` | Heating supply (ITC) | CVV fremlob | C | |
| `cvv_desired_supply_temperature` | Heating desired supply (ITC) | CVV onsket fremlob | C | |
| `cvv_return_temperature` | Heating return (ITC) | CVV retur | C | |
| `system_pressure` | System pressure | Anlaegstryk | bar | Register 7310 |
...
| `ch_desired_inlet_temperature` | Heating desired inlet (CH) | Varme onsket indlob (CH) | C |
| `hc_supply_temperature` | Heating supply (HC) | Varme fremlob (HC) | C |
...
| `flow_sensor_type` | Flow sensor type | Flowmaler type | - |
| `dhw_mode` | DHW mode | BVV mode | - |
| `documented_secondary_pressure` | Documented secondary pressure | Dokumenteret sekundartryk | bar |
...
> **Note:** `hc_supply_temperature` and `hc_return_temperature` show as unavailable if those sensors are not physically connected (Sentio returns the special INVALID_VALUE 0x7FFF = 327.67 C for missing sensors, which is converted to unavailable).
...
| `error_dhw_temperature_high` | DHW temperature high | Brugsvand temperatur hoj |
| `dhw_motor_failure` | DHW motor failure | BVV motorfejl |
| `dhi_sensor_failure` | District heating supply sensor failure | Fjernvarme fremlobsfoler fejl |
| `dho_sensor_failure` | District heating return sensor failure | Fjernvarme returfoler fejl |
| `dhw_sensor_failure` | DHW sensor failure | Brugsvandsfoler fejl |
| `dcw_sensor_failure` | Cold water sensor failure | Koldtvandsfoler fejl |
| `warning_pressure_high` | High pressure warning | Tryk hoj advarsel |
...
| `no_secondary_pressure` | No secondary pressure | Intet sekundartryk |
...
| `itc_hs_sensor_failure` | Heating supply sensor failure (ITC) | CVV fremlobsfoler fejl |
| `itc_hr_sensor_failure` | Heating return sensor failure (ITC) | CVV returfoler fejl |
| `outdoor_sensor_failure` | Outdoor sensor failure | Udetemperaturfoler fejl |
...
Major update: extensive register-map corrections based on the official DHW-21x Modbus specification (TV414 v11), 10+ new sensors, 16 new binary sensors, Danish language support, and INVALID_VALUE handling.
...
**Bug fixes:**
- Sensors with INVALID_VALUE (0x7FFF = 327.67 C per Modbus spec) now show as unavailable instead of 327.7 C - affects HC sensors when not physically connected
- Removed `source_power` calculated field (unsupportable estimate)

### v0.1.15 - previous release

- Initial setup scans for the Modbus port automatically
- Setup form no longer asks for port during first install
- Detected port is saved in the config entry
- Options still expose `port` (0 = auto) for override/re-scan
